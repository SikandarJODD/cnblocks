...
<div class="w-fit">
    ...
    <div class="relative">
      ...
      {#if isActive("system")}
        <span
          ...
          class="bg-foreground absolute -bottom-0.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full"
        ></span>
      {/if}
    </div>
  ...
  {#if hoveredTheme}
    <div
      aria-live="polite"
      class="text-muted-foreground w-fit text-xs leading-none"
