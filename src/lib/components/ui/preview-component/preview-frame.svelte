<script lang="ts">
  ...
  let currentRegistry: string = $derived(
    ...
    <Frame.Header class="px-3 py-1.5 sm:px-4">
      <div class="flex flex-wrap items-center justify-between gap-2.5">
        ...
        <div class="flex items-center gap-1.5">
          {#if hasCode}
            <div
              class="bg-muted/60 inline-flex items-center gap-0.5 rounded-md border p-0.5"
            >
              <Button
            ...
            >
              <Add.Root item={resolvedInstallUrl} withoutRegistry>
                <Add.Group class="h-8 w-88 max-w-full">
                  <Add.Button
                    class="h-8 min-w-0 md:pl-2 md:pr-2 [&>div]:size-8"
                  />
