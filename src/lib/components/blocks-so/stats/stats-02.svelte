...
<div class="flex items-center justify-center p-10">
  <div
    class="grid grid-cols-1 divide-y bg-border divide-border overflow-hidden rounded-lg md:grid-cols-3 md:divide-x md:divide-y-0"
  >
    {#each stats as item (item.metric)}
      <Card class="rounded-none border-0 shadow-sm py-0">
        <CardContent class="p-4 sm:p-6">
          ...
          <div class="mt-1 flex items-baseline gap-2 md:block lg:flex">
            <div
              class="flex items-baseline text-2xl font-semibold text-primary"
            >
              {item.current}
