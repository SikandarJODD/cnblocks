...
function verifyManifestFile(manifestPath, expectedName) {
	...
	assert(Array.isArray(manifest.files) && manifest.files.length > 0, `Missing files in ${manifestPath}`);
}
...
function main() {
	...
	for (const item of registry.items) {
		...
		assert(mappedNamespace === expectedNamespace, `Namespace mismatch for ${item.name}: expected ${expectedNamespace}, got ${mappedNamespace}`);
	...
	assert(Object.keys(namespaceMap).length === registry.items.length, "Namespace map count does not match registry item count");
	...
	assert(veilCount - veilStatsCount === 56, `Expected 56 non-stats Veil items, found ${veilCount - veilStatsCount}`);
	assert(countJsonFiles(path.join(STATIC_DIR, "r")) === namespaceCounts.r, "static/r count mismatch");
	assert(countJsonFiles(path.join(STATIC_DIR, "v")) === namespaceCounts.v, "static/v count mismatch");
	assert(countJsonFiles(path.join(STATIC_DIR, "m")) === namespaceCounts.m, "static/m count mismatch");
